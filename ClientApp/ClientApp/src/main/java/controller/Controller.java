...
public class Controller extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action != null && action.equals("register")) {
		     ...
		     if (name != null && surname != null && username != null && hashedPassword != null) {
		    	 Registration registration;
		    	 if(driverLicense.length() == 0) {
		    		 registration = new Registration(name, surname, username, hashedPassword, email, phone, avatar, idCard, passport, null);

		    	 }else {
		    		 registration = new Registration(name, surname, username, hashedPassword, email, phone, avatar, idCard, passport, driverLicense);
		    	 }
			...
			int clientId = clientBean.getClientId(username);
			boolean isBlocked = clientBean.checkIsClientBlocked(clientId);
			...
			if(isBlocked) {
				...
				if(clientBean.login(username, hashedPassword)) {
			...
			boolean result = PersonDAO.updatePassword(session.getAttribute("username").toString(), hashedPassword);
